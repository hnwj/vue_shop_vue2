...
export default {
  ...
  methods: {
    ...
    async showEditDialog (id) {
      ...
      if (res.meta.status !== 200) return this.$message.error('用户数据查询失败')
      console.log('res', res)
    ...
    async removeUserInfo (id) {
      const confirmResult = await this.$confirm('您确定删除此用户吗?', '永久删除该用户', {
      ...
      this.$http.delete(`users/${id}`).then(res => {
        ...
        if (response.meta.status !== 200) return this.$message.error('用户删除失败')
    ...
    async saveRoleInfo () {
      ...
      if (res.meta.status !== 200) return this.$message.error('用户角色分配失败')
      this.$message.error('用户角色分配成功')
      this.getUserList()
